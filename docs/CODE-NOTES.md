...
- **管理者へのメール通知の間隔制御（`mailThrottled`）**: `test`（管理者のテスト送信）以外は**全種類**を
  5分に1通へ間引く。以前は `dm` だけが対象で `apply`/`unblock` は無制限だったため、停止中ユーザーが
  「解除申請」を連打すると管理者のメールへ無制限に送れた（通知チャネルへの DoS）。
  申請そのものの Firestore 保存は間引きと無関係に成功する。
- **進捗の保存（`stores[CERT_KEY]`）**: `FieldPath` で当該資格のサブストアだけを丸ごと置換する。
  他資格のサブストアを保持しつつ、リセット（空ストア書込）で古い履歴キーが残らないようにするため（deep-merge を避ける）。
- **模試履歴の集計**: 合否は `e.pass`（真偽）で判定する。`e.ok` は「正解数（整数）」なので合格数の判定に使うと
  1問正解でも真になる。合格率の母数は「本番形式（フル）＝問題数が `examN` の模試」に揃える。
- **オフライン用の承認キャッシュ**: `sfq_access_<uid>` は `{v,ts}` 形式で30日で失効させる
  （オフラインのまま無期限に素通りさせない）。旧形式の素の `'approved'` は後方互換で素通しし、
  次のオンラインログインで新形式へ移行する。
