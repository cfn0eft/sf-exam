...
let QDOMAIN={};
let DOMAIN_BY={};
let DOMAIN_TIME_PREFIX='', DOMAIN_LEGACY_TIME={};
...
function timeDomainDef(key){
  if(DOMAIN_LEGACY_TIME[key])return DOMAIN_LEGACY_TIME[key];
  const code=DOMAIN_TIME_PREFIX&&key.startsWith(DOMAIN_TIME_PREFIX)?key.slice(DOMAIN_TIME_PREFIX.length):key;
  return DOMAIN_BY[code]||{code:key,name:'未分類: '+key,emoji:''};
}
...
function recStudyTime(domain,ok,sec){
  ...
  // Historical aggregate time cannot be reclassified by question ID.
  if(domain){const key=DOMAIN_TIME_PREFIX+domain;const d=store.time.dom[key]||(store.time.dom[key]={sec:0,n:0});d.sec+=sec;d.n++;}
...
async function loadCertData(){
  ...
  DOMAIN_TIME_PREFIX=dom&&typeof dom.timeKeyPrefix==='string'?dom.timeKeyPrefix:'';
  DOMAIN_LEGACY_TIME=Object.fromEntries((dom&&Array.isArray(dom.legacyTimeDomains)?dom.legacyTimeDomains:[]).map(d=>[d.code,d]));
  ...
  if(qGeneration!==window.SFQ_BANK.generation()){QDATA=[];throw new Error('利用状態が変わりました。再読み込みしてください。');}
...
document.addEventListener('DOMContentLoaded',initializeCertData);
window.addEventListener('sfq-bank-access',()=>{
  // Reload destroys all question arrays, rendered text, pending exams and closures.
  // Stored progress and the ID-only interrupted-exam record remain intact.
  QDATA=[];allQ=[];filtQ=[];
  if(certDataLoaded){location.reload();return;}
  if(document.readyState!=='loading')initializeCertData();
});
...
function rootCauseHTML(){
  ...
  if(t.dom){
    ...
    if(per.length>=2){per.sort((a,b)=>b.avg-a.avg);const s=per[0];if(s.avg>=25)insights.push('🐢 時間がかかる分野: <b>'+escH(timeDomainDef(s.code).name)+'</b>（1問平均 '+Math.round(s.avg)+'秒）。反射的に解けるよう反復を。');}
  }
...
function timeHTML(){
  ...
  const doms=Object.entries(t.dom||{}).map(([code,v])=>[code,(v&&v.sec)||0]).filter(x=>x[1]>0).sort((a,b)=>b[1]-a[1]).slice(0,DOMAIN_TIME_PREFIX?undefined:5);
  const maxD=doms.length?doms[0][1]:1;let domBars='';
  doms.forEach(pair=>{const def=timeDomainDef(pair[0]);domBars+='<div class="an-row an-time-row"><span class="an-lab">'+escH(def.emoji+' '+def.name)+'</span><div class="an-bw"><div class="an-bf" style="width:'+Math.round(pair[1]/maxD*100)+'%;background:var(--primary)"></div></div><span class="an-pct">'+fmtM(pair[1])+'</span></div>';});
