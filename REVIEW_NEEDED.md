...
- ✅ 解決済み（2026-09-22）: Agentforce Specialist の公式試験ガイドを確認。
  公式5分野は AI Agents 35% / Prompt Engineering 20% / Data 360 Fundamentals 20% / Development Lifecycle 20% / Multi-Agent Interoperability 5%。現在の `deploy` 10% と `gov` 10% は Development Lifecycle の内部的な学習分類であり、合計20%を維持する。データベース側の設問分類を移行しない限り統合しない。
