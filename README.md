...
| | 機能 | 説明 |
|---|---|---|
| 📝 | **学習モード** | 1 問ずつ解いて、選択肢ごとの詳しい解説をその場で確認 |
| ⏱️ | **試験モード** | 公式比率で 60 問を出題・制限時間つき・分野別スコアで採点 |
...
| **Salesforce 認定 Platform アプリケーションビルダー** | — | **445 問** | タイソン 276 / 生成 169 | 5 分野 | 83 語 | 63% |
...
├─ cloud-sync.js           # クラウド同期（Auth + Firestore）
...
- 実際のアクセス制御は **Firestore セキュリティルール**（`progress/{uid}` を本人＋管理者のみ）で行います。
