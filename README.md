...
| **Salesforce 認定 Agentforce Specialist** | AI-201 | **201 問** | 6 分野 | 26 語 | — | 72% |
...
Agentforceは2026-09-26に201問へ更新しました。Jpnshikenの最新154問版から、未処理セッションの即時トレース、サブエージェント誤ルーティング、エージェントユーザーの権限、分類説明とスコープ、Record Snapshot、アクション指示、Trust Layer、Sales EmailsのFLS、Agent Scriptの文字列リスト、変更セット移送、プロンプト版の不変性、意図クラスタ条件、画面フローからのプロンプト呼び出し、Intelligent Contextのサンプル評価、取得チャンクによるRAG障害切り分け、Agentforce Sales Managementの現行用途、Service Agentの有人引き継ぎ、関連リストグラウンディング、Resolved Promptのプレビューを累計19問追加し、公式仕様と異なる出典回答は修正または不採用にしました。取得した112問について、既存問題との重複、隣接製品、公式根拠不足、保留を区別しています。[154問版の照合記録](docs/agentforce-jpn154-source-intake-2026-09-26.json)、[101問の照合記録](docs/agentforce-jpn101-source-intake-2026-09-14.json)、[106問版の照合記録](docs/agentforce-jpn106-source-intake-2026-09-14.json)を参照してください。残り42問の本文取得と正誤監査は未完了です。
