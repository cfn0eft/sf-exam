...
- 不正利用の確認とアカウント管理のため、アカウントのログイン時（承認待ち・停止中を含む）に
...
- 管理者は `broadcast/session-control` を通じて、管理者以外のログイン中セッションへ一斉ログアウトを通知できます。
  オフライン端末も再接続時に命令を確認し、命令後にログインし直した新しいセッションはログアウト対象になりません。
