...
if(!process.env.SFQ_PRIVATE_DATA_ROOT)throw new Error('Link checks require SFQ_PRIVATE_DATA_ROOT');
...
fs.readdirSync(path.join(ROOT, 'certifications')).forEach((slug) => {
  const p = path.join(process.env.SFQ_PRIVATE_DATA_ROOT, 'certifications', slug, 'data', 'questions.json');
