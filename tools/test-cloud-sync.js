...
t('専門資格の利用許可: 明示的な許可と利用承認の両方が必要で、解除・ログアウト時に失効する', () => {
  ...
  for (const data of [null, {}, { access: 'approved' }, { access: 'approved', specialistAccess: false },
    { access: 'approved', specialistAccess: 'true' }, { access: 'pending', specialistAccess: true },
    { access: 'blocked', specialistAccess: true }]) {
    T.publishSpecialistAccess(data);
    eq(sandbox.SFQ_SPECIALIST_ACCESS, false);
  }
});
...
t('Firestoreルール: 専門資格の利用許可を本人が作成・変更できない', () => {
  ...
  ok(/request.resource.data.keys\(\).hasOnly\(selfKeys\(\)\)/.test(rulesSrc));
  ok(/affectedKeys\(\).hasOnly\(selfKeys\(\)\)/.test(rulesSrc));
});
...
t('分野別時間集計: 改訂前の時間を新分類へ加算しない', () => {
  const rows=[{store:{time:{dom:{app:{sec:60},change:{sec:30},'sales26:app':{sec:20},'sales26:ai':{sec:40}}}}},
    {store:{time:{dom:{'sales26:app':{sec:10},'sales26:bad':{sec:5},bad:{sec:-5}}}}}];
  const input=JSON.stringify(rows);
  const result=T.aggregateDomainTime(rows,{defs:[{code:'app'},{code:'ai'}],timeKeyPrefix:'sales26:'});
  eq(result.current.app,30);eq(result.current.ai,40);
  eq(result.legacy.app,60);eq(result.legacy.change,30);eq(result.legacy['sales26:bad'],5);
  eq(Object.values(result.current).reduce((a,b)=>a+b,0)+Object.values(result.legacy).reduce((a,b)=>a+b,0),165);
  eq(JSON.stringify(rows),input,'同期元のデータを変更しない');
});

t('分野別時間集計: 未改訂資格のキーと欠損データに対応する', () => {
  const result=T.aggregateDomainTime([{store:{time:{dom:{cfg:{sec:12},old:{sec:9},invalid:{sec:'30'}}}}},{store:{}}],{defs:[{code:'cfg'}]});
  eq(result.current.cfg,12);eq(result.legacy.old,9);eq(result.legacy.invalid,undefined);
  eq(Object.keys(T.aggregateDomainTime([],{defs:[]}).current).length,0);
});
