...
function validateCert(slug, figKeys) {
  ...
  let questions;
  try { questions = readBank(slug); }
  catch (e) { err('questions.json が読めない: ' + e.message); return; }
  if (!Array.isArray(questions)) { err('questions.json が配列でない'); return; }
  try {
    const meta = readJSON(path.join(ROOT, 'question-catalog.json'))[slug];
    if (!meta || meta.count !== questions.length || meta.items.length !== questions.length || !/^[a-f0-9]{64}$/.test(meta.sha256)) err(slug + ' 公開メタデータが不正');
    if (process.env.SFQ_PRIVATE_DATA_ROOT && meta.sha256 !== require('crypto').createHash('sha256').update(JSON.stringify(questions)).digest('hex')) err(slug + ' 原本と公開メタデータが不一致。build-question-bank.jsを実行してください');
  } catch (e) { err('公開メタデータの検証失敗: ' + e.message); }
  ...
  if (process.env.SFQ_PRIVATE_DATA_ROOT) checkCrossSourceDuplicates(questions);
  else info('  公開CI: 架空問題による構造・参照検証（本文品質は非公開原本で検証）');
...
function validateLanding() {
  ...
  certs.forEach((c) => {
    ...
    let qs = [], vocab = [];
    try { qs = readBank(c.slug); } catch (e) { err(e.message); return; }
...
function validateBlueprint() {
  ...
  fs.readdirSync(path.join(ROOT, 'certifications')).forEach((slug) => {
    ...
    let qs, dom;
    try { qs = readBank(slug); dom = readJSON(path.join(dir, 'domains.json')); }
    catch (e) { return; }
...
function validateSyntax() {
  info('\n== JS 構文 ==');
  const files = ['question-bank.js', 'quiz-engine.js', 'cloud-sync.js', 'analytics.js', 'changelog.js', 'figures.js', 'progression.js', 'firebase-config.js', 'sw.js']
