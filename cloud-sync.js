(function () {
  ...
  function injectStyle() {
    var css = '' +
      ...
      '.sfqc-dom-legacy>span:first-child{flex:1;min-width:0;overflow-wrap:anywhere}.sfqc-dom-legacy>span:last-child{flex-shrink:0}' +
      ...
      '.sfqc-adminwrap{overflow:auto}.sfqc-adminbody{flex:0 0 auto;overflow:visible}.sfqc-toolbar,.sfqc-tabs,.sfqc-detail th{position:static}' +
      ...
      '.sfqc-field{min-height:46px;background:#fff;color:#17211f;border:1px solid #cfd5cf;border-radius:6px;padding:11px 12px}' +
      '.sfqc-field:focus{border-color:#167565;box-shadow:0 0 0 2px rgba(22,117,101,.14)}' +
  ...
  function publishSpecialistAccess(data) {
    var allowed = !!(data && data.access === 'approved' && data.specialistAccess === true);
    if (window.SFQ_SPECIALIST_ACCESS === allowed) return;
    window.SFQ_SPECIALIST_ACCESS = allowed;
  ...
  window.__cloudSetElective = function (slug) {
    if (!window.SFQ_PROG || !window.SFQ_PROG.canChoose(slug)) return Promise.reject(new Error('この資格は選択できません'));
    if (!currentUser || !db) {
  ...
  window.__cloudSave = function () {
    ...
    if (window.SFQ_PROG && window.SFQ_PROG.restrictedOf(CERT_KEY)) return;
    setStatus('保存中…');
    cloudDirty = true;
    ...
    saveTimer = setTimeout(function () {
      ...
      if (window.SFQ_PROG && window.SFQ_PROG.restrictedOf(CERT_KEY)) { cloudDirty = false; return; }
  ...
  function adminDashboardHTML() {
    ...
    if (dom) {
      ...
      var domainTimes = aggregateDomainTime(adminRows.filter(function(r){return r.cert === dcert;}), dom);
      Object.keys(domainTimes.current).forEach(function(dc){
        var da=domAgg[dc]||(domAgg[dc]={c:0,t:0,sec:0});da.sec=domainTimes.current[dc];
      });
      var defs = dom.defs || [];
      var dbars = '';
      ...
      var legacyDefs = dom.legacyTimeDomains || [];
      var legacyRows = Object.keys(domainTimes.legacy).map(function(code){
        var def=legacyDefs.find(function(d){return d.code===code;});
        return '<div class="sfqc-dom sfqc-dom-legacy"><span>'+esc(def?def.name:('未分類: '+code))+'</span><span>'+esc(fmtDur(domainTimes.legacy[code]))+'</span></div>';
      }).join('');
  ...
  var dashDomCache = {};
  function aggregateDomainTime(rows, dom) {
    var result={current:{},legacy:{}};
    var prefix=dom.timeKeyPrefix||'';
    var codes=(dom.defs||[]).map(function(d){return d.code;});
    rows.forEach(function(r){
      var buckets=(r.store&&r.store.time&&r.store.time.dom)||{};
      Object.keys(buckets).forEach(function(key){
        var sec=buckets[key]&&buckets[key].sec;if(typeof sec!=='number'||!isFinite(sec)||sec<=0)return;
        var current=!prefix||key.indexOf(prefix)===0;
        var code=current?key.slice(prefix.length):key;
        var target=current&&codes.indexOf(code)>=0?result.current:result.legacy;
        var outKey=target===result.current?code:key;
        target[outKey]=(target[outKey]||0)+sec;
      });
    });
    return result;
  }
  function getDashDom(slug) {
    if (slug === CERT_KEY && typeof QDATA !== 'undefined' && QDATA && QDATA.length &&
      ...
      return { defs: DOMAIN_DEFS, domBy: domBy,
        timeKeyPrefix:typeof DOMAIN_TIME_PREFIX==='string'?DOMAIN_TIME_PREFIX:'',
        legacyTimeDomains:typeof DOMAIN_LEGACY_TIME==='object'?Object.values(DOMAIN_LEGACY_TIME):[] };
    }
    ...
    Promise.all([grab(base + slug + '/data/domains.json'), window.SFQ_BANK.load(slug)]).then(function (res) {
      ...
      dashDomCache[slug] = { loaded: true, defs: defs, domBy: domBy,
        timeKeyPrefix:domj&&domj.timeKeyPrefix||'',legacyTimeDomains:domj&&domj.legacyTimeDomains||[] };
      renderAdmin();
  ...
  function renderAdmin() {
    ...
    if (adminTab === 'overview') {
      ...
      list.forEach(function (u, i) {
        ...
        var specialistBtn = '<button class="sfqc-act-maint' + (u.specialistAccess ? ' on' : '') + '" data-specialist-uid="' + esc(u.uid) + '" data-specialist-name="' + esc(u.name) + '" data-specialist-state="' + (u.specialistAccess ? '0' : '1') + '" title="コア資格以降の利用許可。ログイン承認と資格取得順の条件も適用されます">' +
          (u.specialistAccess ? '🔓 専門資格の許可を解除' : '🔒 専門資格を許可') + '</button>';
    ...
    body.querySelectorAll('[data-specialist-uid]').forEach(function (b) {
      b.addEventListener('click', function () { setSpecialistAccess(b.getAttribute('data-specialist-uid'), b.getAttribute('data-specialist-name'), b.getAttribute('data-specialist-state') === '1'); });
    });
  ...
  function setSpecialistAccess(uid, name, on) {
    if (!isAdmin || !db) return;
    var verb = on ? '許可' : '解除';
    if (!confirm('「' + name + '」のコア資格以降の利用許可を' + (on ? '付与' : '解除') + 'します。よろしいですか？')) return;
    db.collection(COLLECTION).doc(uid).set({ specialistAccess: on, updated: Date.now() }, { merge: true })
      .then(function () {
        var u = findUser(uid); if (u) u.specialistAccess = on;
        logAdmin('専門資格の利用' + verb, name);
        toastSafe('「' + name + '」の専門資格の利用許可を' + (on ? '付与' : '解除') + 'しました'); renderAdmin();
      })
      .catch(function (e) { alert('変更に失敗しました: ' + (e && e.message)); });
  }
  ...
  function init() {
    ...
    var localHost = _h === 'localhost' || _h === '127.0.0.1' || _h === '::1' || _h === '';
    var emulatorMode = localHost && window.SFQ_EMULATOR === true;
    if (localHost && !emulatorMode) {
      hideOverlay();
    ...
    try {
      firebase.initializeApp(emulatorMode ? Object.assign({}, CFG, {projectId:'demo-sfq-bank',apiKey:'demo-key'}) : CFG);
      auth = firebase.auth();
      db = firebase.firestore();
      if (emulatorMode) {
        auth.useEmulator('http://127.0.0.1:9199', {disableWarnings:true});
        db.useEmulator('127.0.0.1', 8189);
      }
      if (window.SFQ_BANK) window.SFQ_BANK.initialize(auth, db);
    } catch (e) {
      hideOverlay();
