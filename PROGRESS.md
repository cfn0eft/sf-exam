...
2026-09-26 | design | 全8資格・ログイン確認の準備画面を preparation.css に統一。光の背景・浮かぶ本とカード・周回する光・波形インジケーターを追加。管理画面の集計待ちも統一。エラー時と reduced-motion は動きを停止。v208の公開対象（ユーザー承認済み） | engine 46件・cloud-sync 61件、progression/question-bank、公開ビルド10ページを確認。320/390px・明暗・エラー・動きを減らすスタイルをローカルブラウザーで確認。既存の用語タイトル重複警告1件は変更なし

2026-09-26 | fix | 起動直後のログインフォーム表示を認証状態の復元完了後へ移動。確認中は準備画面、未ログイン確定時のみフォームを表示。15秒経過時・認証確認エラー時は再読込案内を表示し、時間経過による未ログイン判定をしない。v207の公開対象 | 認証復元・未ログイン・通信待ち・確認エラーの回帰テストを追加。既存の別作業は公開差分から除外

2026-09-26 | release | 利用者の承認により、読み込み途中の資格ロック表示修正を公開準備。お知らせを追加し版数をv206へ更新。既存の資格名・試験コードの未コミット変更4ファイルは公開差分から除外して保全 | 公開対象の検証後にcommit・push・Pages配信を確認する

2026-09-26 | fix | 読み込み途中に資格の未解除表示が出る問題を修正。クラウドの進捗・管理者判定・専門資格許可がそろってからロックを判定し、問題と進捗の到着順にかかわらず準備画面を維持。取得順・利用承認・専門資格制限は維持。既存のHTML4ファイルと未追跡作業は保全 | validate-data exit 0（既存の用語名重複警告1件）/ engine 46件成功 / cloud-sync 60件成功 / progression・question-bank成功。版数更新・commit・push・本番公開は未実施
